...
// Fungsi ambil semua periode masuk unik
function getPeriodeSiswa() {
    global $conn;
    $result = mysqli_query($conn, "SELECT DISTINCT periode_masuk FROM tb_siswa ORDER BY periode_masuk DESC");
    $rows = [];
    while ($row = mysqli_fetch_assoc($result)) {
        $rows[] = $row;
    }
...
// Fungsi ambil data siswa (opsional dengan filter periode)
function getSiswa($periode = '') {
    global $conn;
    $query = "SELECT * FROM tb_siswa";
    if ($periode !== '') {
        $query .= " WHERE TRIM(periode_masuk) = '$periode'";
    }
    $result = mysqli_query($conn, $query);
    $rows = [];
    while ($row = mysqli_fetch_assoc($result)) {
